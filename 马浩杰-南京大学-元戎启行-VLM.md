...
### **项目一: 基于VLM架构的二段式端到端自动驾驶(元戎启行)**
  - 项目背景：构建统一的多模态自动驾驶大模型，旨在实现从感知理解到决策规划的端到端闭环, 构建具备场景理解，行为决策，风险评估能力的统一模型
  - 技术细节：
    - 模型整体架构
      - 初版基座模型采用Qwen2-VL 7B架构，后续砍到0.5B.
      - 输入为BEV特征,导航地图，感知地图, agent轨迹等多模态信息，通过每个模态独有的encoder，编码为bev token, sdmap token, agent token等.
      - 输出为场景描述、轨迹规划、限速信息、纵横向规划、风险标志, 引入COT输出设计，要求模型在生成轨迹前先输出场景语义描述,关键障碍物与意图预测.
    - 预训练
      - 构建了异构数据混合训练，将通用视觉-语言数据集与自动驾驶数据集（大规模BEV-描述对, 描述对包含3d bbox, occ等描述对）进行混合训练
      - 下一代端到端系统开发，考虑结合Masked World Modeling (MWM) 思想，让模型在未标注的海量路测视频中学习物理规律，提高模型的空间感知能力, 联合分布输出未来frame和action.
    ...
    - 数据方面: 
      - 构建了video级别的感知和规控tag, 包含cutin, junction, lane change, overline等不同的驾驶场景.
      - 车端部署VLM蒸馏模型用作场景分类，进行corner case采集,包含异形障碍物，极端道路结构(窄路，环岛)，交互博弈（强行加塞, 无保护左转）
    - 车端应用， 和二段式planer构成典型的快慢系统，处理 90% 以上的常规驾驶, 当慢系统发现复杂场景(施工路段, 收费站etc通行等)时，会修改快系统的输入参数或直接接管决策
...
### **项目二: 3D感知与OCC感知(元戎启行)**
  ...
  - 3D occ
    - 项目背景：基于3D感知的OCC识别，用于识别不同类型的动态和静态占用物（水马、围栏、锥桶等）
    - 模型架构：
      - 全卷积的head，用于做3D分割，判断是否占据, 占据物体类别等信息
      - 针对雨花误检问题，增加noise类别预测
      - 针对80-160m范围，添加远端occ head.
      - 动静态occ head实现，输出occ flow(vx, vy)
...
### **项目五: 基于Azure ML的集群监控系统**
  - 项目描述：针对微软o365团队内部存储计算集群 Cosmos，搭建了一套从异常监测到自动化运维的闭环系统，旨在降低集群宕机风险并提升任务作业的稳定性。
  - 核心贡献：
    - 多维指标监控：集成 Cosmos QPS，RT，失败率等, 服务器底层硬件（CPU、磁盘 I/O、网络丢包）与系统级故障日志，构建了高维时序特征库。
    - 异常检测架构：接入 Azure Time Series Anomaly Detection 服务，利用其内置的SR-CNN(Spectral Residual-CNN)+孤立森林算法检测，实现了对非平稳时序数据的实时监控。相较于传统阈值告警，漏报率降低了 18%
